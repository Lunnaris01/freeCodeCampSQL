...
echo "$($PSQL "SELECT COUNT(*) AS twoplus_goals FROM games WHERE winner_goals>2")"
...
echo "$($PSQL "SELECT t.name FROM teams t LEFT JOIN games g ON g.winner_id=t.team_id OR g.opponent_id=t.team_id WHERE g.year=2014 AND g.round='Eighth-Final' ORDER BY t.name")"
